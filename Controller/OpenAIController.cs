...
public class OpenAIController : ControllerBase
{
    ...
    public async Task<ActionResult<string>> GenerateResponse([FromBody] UserInputModel inputModel)
    {
        ...
        // Check if the conversation already has 20 questions
        int questionCount = await _context.Questions
            .CountAsync(q => q.Conversation == conversation);

        if (questionCount >= 20)
        {
            return BadRequest("The conversation has reached the maximum number of 20 questions.");
        }
